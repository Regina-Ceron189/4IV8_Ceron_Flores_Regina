...
function validarCompra(datos) {
    ...
    if (!datos.usuario_id) {
        ...
        errores.push('El ID del usuario debe ser un nÃºmero entero positivo');
    }
    ...
    if (!datos.producto_id) {
        ...
        errores.push('El ID del producto debe ser un nÃºmero entero positivo');
    }

    if (datos.cantidad !== undefined) {
        const cant = Number(datos.cantidad);
        if (!Number.isInteger(cant) || cant <= 0) {
            errores.push('La cantidad debe ser un nÃºmero entero mayor que 0');
        }
    }
...
// - Permiten escribir consultas mÃ¡s cortas y legibles
...
router.get('/:id', async (req, res) => {
    try {
        // Verificar que el parÃ¡metro no sea "usuario" (para evitar
